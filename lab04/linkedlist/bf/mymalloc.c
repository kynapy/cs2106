    ...
    if (_memlist == NULL) {
        ...
        do {
            ...
            if (data->status == 'u' && data->memsize >= size) {
                if (data->memsize - size < closest - size) {
                    closest = data->memsize;
                    bestFit = trav;
                }
            }
        } while (trav != NULL);
        ...
        // Returning best fit
        if (bestFit->pdata->memsize != size) {
            TData *newData = malloc(sizeof(TData));
            newData->memsize = bestFit->pdata->memsize - size;
            newData->start_address = bestFit->pdata->start_address + size;
            newData->status = 'u';
            TNode *newNode = make_node(newData->start_address, newData);
            insert_node(&_memlist, newNode, 0);
        }
        bestFit->pdata->status = 'o';
        bestFit->pdata->memsize = size;
        return &_heap[bestFit->pdata->start_address];
    }
}
